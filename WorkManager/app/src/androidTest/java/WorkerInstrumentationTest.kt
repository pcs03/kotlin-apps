...
class WorkerInstrumentationTest {
    ...
    fun cleanupWorker_doWork_resultSuccess() {
        ...
        runBlocking {
            ...
            assertTrue(workResult is ListenableWorker.Result.Success)
        }
    }
    ...
    fun blurWorker_doWork_resultSuccessReturnsUri() {
        val worker = TestListenableWorkerBuilder<BlurWorker>(context)
            .setInputData(workDataOf(mockInputUri))
            .build()

        runBlocking {
            val workResult = worker.doWork()
            val resultUri = workResult.outputData.getString(KEY_IMAGE_URI)

            assertTrue(workResult is ListenableWorker.Result.Success)
            assertTrue(workResult.outputData.keyValueMap.containsKey(KEY_IMAGE_URI))
            assertTrue(
    ...
    @Test
    fun saveImageToFileWorker_doWork_resultSuccessFileSaved() {
